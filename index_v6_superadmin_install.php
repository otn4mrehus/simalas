...
if (isset($_POST['update_presensi'])) {
    ...
    if ($jam_masuk) {
        ...
        if (strtotime($jam_masuk) > strtotime($WaktuMasuk)) {
            $status_masuk = 'terlambat';
            $selisih_detik = strtotime($jam_masuk) - strtotime($WaktuMasuk);
            ...
            if ($selisih_menit >= 60) {
                ...
                $keterangan_terlambat = "$selisih_menit Menit";
            }
        }
    }
    ...
    if ($jam_pulang) {
        if (strtotime($jam_pulang) < strtotime($WaktuPulang)) {
            $status_pulang = 'cepat';
            $selisih_detik = strtotime($WaktuPulang) - strtotime($jam_pulang);
            $selisih_menit = round($selisih_detik / 60);
        ...
        if ($jenis == 'masuk') {
            ...
            if ($row_presensi && $row_presensi['jam_masuk']) {
                ...
                if (isset($_FILES['foto']) && $_FILES['foto']['error'] == UPLOAD_ERR_OK) {
                    ...
                    if (strtotime($waktu) > strtotime($WaktuMasuk)) {
					    $status_masuk = 'terlambat';
					//   $selisih_detik = strtotime($waktu) - strtotime('07:30:00');
                        $selisih_detik = strtotime($waktu) - strtotime(($WaktuMasuk));
					    $selisih_menit = round($selisih_detik / 60);
                ...
                if (isset($_FILES['foto']) && $_FILES['foto']['error'] == UPLOAD_ERR_OK) {
                    ...
                    if (move_uploaded_file($foto['tmp_name'], $targetFile)) {
                        ...
                        if (compressImage($targetFile, $targetFile, 60)) {
                            ...
                            if (strtotime($waktu) < strtotime($WaktuPulang)) {
                                $status_pulang = 'cepat';
                            //  $selisih_detik = strtotime('15:30:00') - strtotime($waktu);
                                $selisih_detik = strtotime($WaktuPulang) - strtotime($waktu);
                                $selisih_menit = round($selisih_detik / 60);
                                ...
                                <tr>
				                    ...
				                    <td>
				                        <?php if ($row['foto_masuk']): ?>
				                            <img src="uploads/foto/masuk/<?php echo $row['foto_masuk']; ?>" class="foto-presensi">
				                        <?php else: ?>
				                            -
				                        <?php endif; ?>
				                    </td>
				                    ...
				                    <td>
				                        <?php if ($row['foto_pulang']): ?>
				                            <img src="uploads/foto/pulang/<?php echo $row['foto_pulang']; ?>" class="foto-presensi">
				                        <?php else: ?>
				                            -
				                        <?php endif; ?>
				                    </td>
