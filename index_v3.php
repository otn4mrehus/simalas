...
// Koneksi ke database
$host = "127.0.0.1";  //Gunakan HOST CONTAINER DATABSENYA jika mengggunakan Docker Server
$user = "root";
$pass = "";
$db = "presensi_siswa";
...
if ($result->num_rows == 0) {
    ...
    $conn->query("CREATE DATABASE IF NOT EXISTS $db DEFAULT CHARACTER SET utf8 COLLATE utf8_unicode_ci");
    ...
    // Insert data default untuk tabel siswa
    $conn->query("INSERT INTO siswa (nis, nama, password, kelas_id, kelas) VALUES 
    ...
    // Insert data default untuk tabel pengaturan
    $conn->query("INSERT INTO pengaturan (latitude, longitude, radius) VALUES ('-6.41050000', '106.84400000', 100)");
            ...
            if ($row_presensi && $row_presensi['jam_masuk']) {
                ...
                if (isset($_FILES['foto']) && $_FILES['foto']['error'] == UPLOAD_ERR_OK) {
					...
					if (strtotime($waktu) > strtotime('07:30:00')) {
					    ...
					    $selisih_detik = strtotime($waktu) - strtotime('07:30:00');
					    $selisih_menit = round($selisih_detik / 60);

					    if ($selisih_menit >= 60) {
						  $jam = floor($selisih_menit / 60);
						  $menit = $selisih_menit % 60;
						  $keterangan_terlambat = "$jam Jam $menit Menit";
					    } else {
						  $keterangan_terlambat = "T$selisih_menit Menit";
					    }
                    ...
                    if (move_uploaded_file($foto['tmp_name'], $targetFile)) {
                        ...
                        if (compressImage($targetFile, $targetFile, 60)) {
                            ...
                            if (strtotime($waktu) < strtotime('15:30:00')) {
                                $status_pulang = 'cepat';
                                $selisih_detik = strtotime('15:30:00') - strtotime($waktu);
                                $selisih_menit = round($selisih_detik / 60);
                                
                                if ($selisih_menit >= 60) {
                                    $jam = floor($selisih_menit / 60);
                                    $menit = $selisih_menit % 60;
                                    $keterangan_pulang_cepat = "$jam Jam $menit Menit";
                                } else {
                                    $keterangan_pulang_cepat = "$selisih_menit Menit";
                                }
                            }
...
<body>
    ...
    <div class="container">
        ...
        <?php if ($page == 'admin' && isset($_SESSION['admin'])): ?>
            ...
            <!--EDIT TAMBAHAN DATA PRESENSI SISWA -->
		<?php
		    // Total Siswa
		...
		?>
          ...
          <!--EDIT TAMBAHAN DATA PENGAJUAN IJIN -->
		<?php
		    // Total Ijin Siswa
			  ...
			  <?php endif; ?>
                ...
                <div id="terlambat" class="tab-content">
                    <div class="card">
                        ...
                        <?php 
                        $sql = "SELECT p.tanggal, p.nis, s.nama, p.keterangan_terlambat 
                                FROM presensi p 
                        ...
                        <?php if ($result_per_siswa->num_rows > 0): ?>
                            <div class="table-responsive">
                                <table>
                                    ...
                                    <tbody>
                                        <?php while ($row = $result_per_siswa->fetch_assoc()): 
                                            $hadir = $row['hadir'];
                                            $sakit = $row['sakit'];
                                            $ijin = $row['ijin'];
    ...
    <div class="footer">
        <p>© <?php echo date('Y'); ?> Sistem Presensi Kelas XITKJ2  <br/> <h6>Man To Development & Maintenance</h6><h5>masihgurutkj@gmail.com</h5></p>
    </div>
